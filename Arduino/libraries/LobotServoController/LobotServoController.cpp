...
void LobotServoController::sendCMDGetServoPositions()
{
	uint8_t buf[11];
	buf[0] = FRAME_HEADER;
	buf[1] = FRAME_HEADER;
			...
			if(dataCount == dataLength + 2)
			{
				...
				switch(buf[3])
				{
					...
					case CMD_MULT_SERVO_POS_READ:
						servoPos[0] = BYTE_TO_HW(buf[6], buf[7]);
						servoPos[1] = BYTE_TO_HW(buf[9], buf[10]);
						servoPos[2] = BYTE_TO_HW(buf[12], buf[13]);
						servoPos[3] = BYTE_TO_HW(buf[15], buf[16]);
						servoPos[4] = BYTE_TO_HW(buf[18], buf[19]);
						servoPos[5] = BYTE_TO_HW(buf[21], buf[22]);
						isReadServos = true;
						break;
					default:
						break;
...
uint16_t * LobotServoController::readServos()
{
	isReadServos = false;
	sendCMDGetServoPositions();
	while(!isReadServos)
	{
		receiveHandle();
	}
